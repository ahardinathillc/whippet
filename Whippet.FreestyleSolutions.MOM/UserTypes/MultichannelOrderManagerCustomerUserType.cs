...
{
    /// <summary>
    /// Represents the custom <see cref="IUserType"/> for <see cref="MultichannelOrderManagerCustomer"/>.
    /// </summary>
    public class MultichannelOrderManagerCustomerUserType : WhippetUserTypeBase, IUserType
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="MultichannelOrderManagerCustomerUserType"/> class with no arguments.
        /// </summary>
        public MultichannelOrderManagerCustomerUserType()
            : base()
        {
            SqlTypes = new[] { SqlTypeFactory.Int64 };
            ReturnedType = typeof(MultichannelOrderManagerCustomer);
            IsMutable = true;
        }
        ...
        public override object NullSafeGet(DbDataReader rs, string[] names, ISessionImplementor session, object owner)
        {
            ...
            if (rawObj != null)
            {
                rawObj = new MultichannelOrderManagerCustomer(Guid.Empty, Convert.ToInt64(rawObj));
            }
        ...
        public override void NullSafeSet(DbCommand cmd, object value, int index, ISessionImplementor session)
        {
            if (cmd != null)
            {
                if (value == null || !(value is IMultichannelOrderManagerCustomer))
                {
                    ((IDataParameter)(cmd.Parameters[index])).Value = DBNull.Value;
                }
                else
                {
                    ((IDataParameter)(cmd.Parameters[index])).Value = ((IMultichannelOrderManagerCustomer)(value)).CustomerId;
                }
            }
        }
    }
