...
{
    /// <summary>
    /// Represents the custom <see cref="IUserType"/> for <see cref="MultichannelOrderManagerWarehouse"/>.
    /// </summary>
    public class MultichannelOrderManagerWarehouseUserType : WhippetUserTypeBase, IUserType
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="MultichannelOrderManagerWarehouseUserType"/> class with no arguments.
        /// </summary>
        public MultichannelOrderManagerWarehouseUserType()
            : base()
        {
            SqlTypes = new[] { SqlTypeFactory.Int64 };
            ReturnedType = typeof(MultichannelOrderManagerWarehouse);
            IsMutable = true;
        }
        ...
        public override object NullSafeGet(DbDataReader rs, string[] names, ISessionImplementor session, object owner)
        {
            ...
            if (rawObj != null)
            {
                rawObj = new MultichannelOrderManagerWarehouse(Convert.ToInt64(rawObj));
            }
        ...
        public override void NullSafeSet(DbCommand cmd, object value, int index, ISessionImplementor session)
        {
            if (cmd != null)
            {
                if (value == null || !(value is IMultichannelOrderManagerWarehouse))
                {
                    ((IDataParameter)(cmd.Parameters[index])).Value = DBNull.Value;
                }
                else
                {
                    ((IDataParameter)(cmd.Parameters[index])).Value = ((IMultichannelOrderManagerWarehouse)(value)).WarehouseID;
                }
            }
        }
    }
